...
def add_transaction():
    ...
    if type == "sell":
        if coin_details["coins"]>=coins and coin_details["total_value"]>=value_usd:
            cur.execute(f"INSERT INTO transactions (name,symbol,type,value_usd,purchased_price,date,coins,status) VALUES ('{name}','{symbol}','{type}',{value_usd},{purchased_price},'{date}',{coins})")
            conn.commit()
